...
int main() {
    ...
    while (1) {
        ...
        if (result > 0) {
            if (result == 1) {
                // Player A wins
                char win_msg_a[] = "You win, Player A! wanna play again kid??\n";
                char lose_msg_b[] = "Player A wins! wanna play again kid??\n";
            ...
            if (result == 2) {
                // Player B wins
                char win_msg_b[] = "You win, Player B! wanna play again kid??\n";
                char lose_msg_a[] = "Player B wins! wanna play again kid??\n";
            ...
            if (result == 3) {
                ...
                // Receive responses from both players...all these lite
                char response_a[1], response_b[1];
                recv(soc_client_a, response_a, 1, 0);
                recv(soc_client_b, response_b, 1, 0);
            ...
            continue; // Go back to the top of the loop if the game ended
        }
        ...
        send(soc_client_a, wait_prompt, strlen(wait_prompt), 0);
